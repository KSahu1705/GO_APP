...
func (a *App) Initialize(config *config.Config) {
	...
	db, _ := sqlx.Connect(config.DB.Dialect, dbURI)
	fmt.Println(db)
	// if err != nil {
...
func InsertUserData(ctx context.Context, db *sqlx.DB) (*entity.User, error) {
	c := entity.User{}
	fmt.Println("====>",db)
	stmt, err := db.Preparex(queryInsertUserData)

	err = stmt.SelectContext(ctx, &c)
	if err != nil {
		return nil, err
	}
	return &c, nil
